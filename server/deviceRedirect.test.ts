...
const NEXUS_TABLET_USER_AGENT =
  "Mozilla/5.0 (Linux; Android 13; Nexus 9 Build/TP1A) AppleWebKit/537.36 Chrome/125.0.0.0 Safari/537.36";
const KINDLE_TABLET_USER_AGENT =
  "Mozilla/5.0 (Linux; U; en-us; KFAPWI Build/JDQ39) AppleWebKit/535.19 Silk/3.13 Safari/535.19";
...
describe("mobile and tablet redirect", () => {
  ...
  it.each([
    ...
    ["Nexus tablet", NEXUS_TABLET_USER_AGENT],
    ["Kindle tablet", KINDLE_TABLET_USER_AGENT],
  ])("detects %s as a mobile or tablet device", (_device, userAgent) => {
    expect(isMobileOrTablet({ userAgent })).toBe(true);
  });

  it("detects an Android tablet client hint even when the user agent is ambiguous", () => {
    expect(
      isMobileOrTablet({
