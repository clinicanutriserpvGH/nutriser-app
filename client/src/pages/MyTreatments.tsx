          ...
          <div className="space-y-5">
            <div className="bg-[#C5A55A]/5 border border-[#C5A55A]/20 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="text-gray-900 font-bold text-sm">Mis Compras</h3>
              </div>
            ...
            {purchasesLoading ? (
              ...
              <>
                ...
                {(myPurchases?.packages?.length ?? 0) > 0 && (
                  <div>
                    <h4 className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5">
                      ...
                      {myPurchases!.packages.map((pkg: any) => (
                        <div key={pkg.id} className={`bg-white rounded-2xl p-4 border shadow-sm ${
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <p className="text-gray-900 font-bold text-sm">{pkg.programName || pkg.programType}</p>
                              <p className="text-gray-500 text-xs mt-0.5">Precio: ${pkg.price} MXN</p>
                            ...
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${
                              ...
                              pkg.status === 'pending' ? 'bg-yellow-50 text-yellow-700' :
                          ...
                          {pkg.status === 'pending' && (
                            <p className="text-yellow-600 text-xs mt-2">Tu paquete está siendo revisado. Recibirás confirmación por correo.</p>
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-3">
                      {myPurchases!.services.map((svc: any) => (
                        <div key={svc.id} className={`bg-white rounded-2xl p-4 border shadow-sm ${
                          ...
                          svc.status === 'rejected' ? 'border-red-200' :
                          'border-yellow-200'
                        }`}>
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <p className="text-gray-900 font-bold text-sm">{svc.serviceName}</p>
                              {svc.originalPrice && <p className="text-gray-500 text-xs mt-0.5">Precio: ${svc.originalPrice} MXN</p>}
                            ...
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${
                              ...
                              svc.status === 'pending' ? 'bg-yellow-50 text-yellow-700' :
                              'bg-red-50 text-red-700'
                            }`}>
                          ...
                          {svc.status === 'pending' && (
                            <p className="text-yellow-600 text-xs mt-2">Tu servicio está siendo revisado. Recibirás confirmación por correo.</p>
                          )}
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-3">
                      {myPurchases!.coupons.map((c: any) => (
                        <div key={c.id} className={`bg-white rounded-2xl p-4 border shadow-sm ${
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <p className="text-gray-900 font-bold text-sm">{c.promotionTitle}</p>
                            ...
                            <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${
                              ...
                              {c.status === 'approved' ? 'Activo' :
                               ...
                               c.status === 'used' ? 'Usado' :
                               c.status === 'expired' ? 'Vencido' : 'Rechazado'}
                            </span>
                          </div>
                          ...
                          {c.status === 'pending' && (
                            <p className="text-yellow-600 text-xs mt-2">Tu cupón está siendo revisado. Recibirás confirmación por correo.</p>
                          )}
                        </div>
                      ))}
                    </div>
