...
import { Loader2, Gift, Sparkles, Mail, Copy, Check, Upload, Clock, X, ArrowRight } from "lucide-react";
...
export default function PromotionsSection() {
  ...
  const createGiftPurchase = trpc.giftPurchases.create.useMutation({
    onSuccess: () => {
      toast.success("¡Comprobante enviado! El admin verificará y activará tu cupón.");
  ...
  // Timer solo empieza cuando llega al paso 2
  useEffect(() => {
    ...
    <section id="promociones" className="py-20 bg-[#FAF7F2]">
      <div className="container">
        ...
        {isLoading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-12 rounded-lg border-2 border-[#C5A55A]/20 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {promotions.map((promo, index) => (
              <motion.div key={promo.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} className="relative">
                <div className="relative" id={`cupon-${promo.id}`}>
                  <div className="bg-gradient-to-br from-[#C5A55A] to-[#B8963E] rounded-t-2xl overflow-hidden shadow-xl">
                    ...
                    <div className="p-8 relative">
                      ...
                      <h3 className="font-serif text-2xl lg:text-3xl text-white mb-4 pr-20 leading-tight">{promo.title}</h3>
                      {promo.description && <p className="text-white/90 text-sm lg:text-base leading-relaxed mb-6 font-light">{promo.description}</p>}
                  ...
                  <div className="bg-white rounded-b-2xl p-6 shadow-xl border-t-2 border-[#C5A55A]/20">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      <button onClick={() => handleShareWhatsApp(promo.title, promo.description || "", promo.id)} className="flex-1 min-w-[100px] flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white py-2 px-3 rounded-lg transition-all font-semibold text-sm">
                      ...
                      <button onClick={() => handleShareEmail(promo.title, promo.description || "", promo.id)} className="flex-1 min-w-[100px] flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white py-2 px-3 rounded-lg transition-all font-semibold text-sm">
                        <Mail className="w-4 h-4" />Email
                      </button>
                      <button onClick={() => handleCopyLink(promo.id, promo.title, promo.description)} className="flex-1 px-3 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors flex items-center justify-center gap-2">
      ...
      {/* Modal de Compra de Regalo - 2 pasos */}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center px-5 py-3 border-b bg-gray-50">
              <div className={`flex items-center gap-2 text-sm font-semibold ${step === "form" ? "text-[#C5A55A]" : "text-gray-400"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === "form" ? "bg-[#C5A55A] text-white" : "bg-gray-300 text-white"}`}>1</span>
                Tus datos
              </div>
              <div className="flex-1 h-px bg-gray-300 mx-3" />
              <div className={`flex items-center gap-2 text-sm font-semibold ${step === "payment" ? "text-[#C5A55A]" : "text-gray-400"}`}>
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === "payment" ? "bg-[#C5A55A] text-white" : "bg-gray-300 text-white"}`}>2</span>
                Pago y comprobante
              </div>
            </div>
            ...
            {/* PASO 1: Datos del comprador */}
            {step === "form" && (
              <form onSubmit={handleContinueToPayment} className="p-5 space-y-4">
                <p className="text-sm text-gray-600">Ingresa tus datos para continuar con la compra del cupón de regalo.</p>
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => { setGiftModalOpen(false); resetForm(); }} className="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition text-sm">
                    Cancelar
                  </button>
                  <button type="submit" className="flex-1 px-4 py-2.5 bg-[#C5A55A] text-white rounded-lg font-semibold hover:bg-[#B8963E] transition text-sm flex items-center justify-center gap-2">
            ...
            {step === "payment" && (
              <>
                ...
                <div className={`flex items-center gap-2 px-5 py-3 border-b ${timeLeft < 120 ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}>
                  ...
                  <div>
                    <span className={`text-sm font-bold ${timeLeft < 120 ? "text-red-900" : "text-amber-900"}`}>
                      Tiempo para subir comprobante: {formatTime(timeLeft)}
                    </span>
                    <p className={`text-xs ${timeLeft < 120 ? "text-red-700" : "text-amber-700"}`}>
                      Si no subes el comprobante a tiempo, deberás registrarte de nuevo para activar tu cupón.
                    </p>
                  </div>
                </div>
                ...
                <form onSubmit={handleGiftSubmit} className="p-5 space-y-4">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    ...
                    <p className="text-xs text-blue-700 mt-2">Realiza la transferencia y sube el comprobante abajo. Tienes {formatTime(timeLeft)} para completarlo.</p>
                  </div>
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                    <p className="text-xs text-yellow-800">
                      <strong>¿Qué pasa después?</strong> El administrador revisará tu comprobante y activará tu cupón. Recibirás confirmación por email.
                    </p>
                  </div>

                  <div className="flex gap-3 pt-2">
                    <button type="button" onClick={() => setStep("form")} className="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition text-sm">
                      Atrás
                    </button>
