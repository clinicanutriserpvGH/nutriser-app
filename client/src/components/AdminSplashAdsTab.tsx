 ...
 * Las imágenes del admin REEMPLAZAN la slide fija (Monedero/ShopCard).
 * Los cupones activos SIEMPRE aparecen en el pop-up.
 * Botón "Restaurar" para volver a mostrar la slide fija original.
 */
...
export default function AdminSplashAdsTab() {
  ...
  const { data: configInicio, refetch: refetchConfigInicio } = trpc.splashAds.getConfig.useQuery({ type: "inicio" });
  const { data: configTienda, refetch: refetchConfigTienda } = trpc.splashAds.getConfig.useQuery({ type: "tienda" });
  ...
  const setShowDefaultMutation = trpc.splashAds.setShowDefault.useMutation({
    onSuccess: (_data, vars) => {
      toast.success(vars.showDefault
        ? `✅ Slide original restaurada en el pop-up de ${vars.type}`
        : `✅ Slide original ocultada en el pop-up de ${vars.type}`
      );
  ...
  const sections = [
    {
      label: "Pop-ups de Inicio",
      sublabel: "Reemplaza la imagen de la Tienda Nutriser",
      icon: Home,
      ads: inicioAds,
    ...
    },
    {
      ...
      sublabel: "Reemplaza la tarjeta del Monedero Nutriser",
      icon: Store,
      ads: tiendaAds,
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Sube imágenes para los pop-ups. Los cupones activos siempre aparecen automáticamente.
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="relative bg-white rounded-2xl overflow-hidden shadow-2xl max-w-sm w-full">
            ...
            <div className="relative bg-black" style={{ aspectRatio: "9/16", maxHeight: "70vh" }}>
              <img src={previewUrl} alt="Preview del pop-up" className="w-full h-full object-cover" />
        ...
        <div key={type} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          ...
          {ads.length === 0 ? (
            <div className="text-center py-8 text-gray-400 text-sm border border-dashed border-gray-200 rounded-xl">
              No hay publicidades. Sube la primera en el formulario de arriba.
            </div>
          ) : (
